...
async def fetch_upcoming_events():
    ...
    # [FIX] Add a guard clause to prevent API calls without a session ID.
    ...
    try:
        ...
        st.sidebar.error(f"Could not fetch calendar events: {e}")
        st.session_state.upcoming_events = []
...
with chat_col:
    ...
    if not st.session_state.messages:
        ...
        # Fetch events on the first load after login
        asyncio.run(fetch_upcoming_events())
        st.rerun() # Rerun to update the calendar and upcoming events list
...
with calendar_col:
    ...
    calendar_events = [{
        ...
        "allDay": "date" in event.get("start", {}) # Handle all-day events
